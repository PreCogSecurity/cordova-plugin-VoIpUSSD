...
public class VoIpUSSD extends CordovaPlugin {
    ...
    public boolean execute(String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals(ACTION_SEND_SMS)) {
            ...
            try {
                ...
                callbackContext.error(buildError("INVALID_ARGUMENTS", "Error encountered: " + e.getMessage()));
                return false;
            }

            String validationError = validateUssdCode(ussdCode);
            if (validationError != null) {
                callbackContext.error(buildError("INVALID_USSD_CODE", validationError));
                return false;
            }
    ...
    public void onRequestPermissionResult(int requestCode, String[] permissions, int[] grantResults) throws JSONException {
        ...
        if (requestCode == SEND_SMS_REQ_CODE) {
            ...
            try {
                ...
                this.callbackContext.error(buildError("INVALID_ARGUMENTS", "Error encountered: " + e.getMessage()));
                return;
            }

            String validationError = validateUssdCode(ussdCode);
            if (validationError != null) {
                this.callbackContext.error(buildError("INVALID_USSD_CODE", validationError));
                return;
            }
    ...
    /**
     * Validates the USSD code supplied by the JavaScript layer before it is
     * handed to the native dialer.
     *
     * @param ussdCode the code to validate
     * @return a human-readable error message, or {@code null} when the code is valid
     */
    private String validateUssdCode(String ussdCode) {
        if (ussdCode == null || ussdCode.trim().isEmpty()) {
            return "ussdCode must be a non-empty string";
        }
        return null;
    }

    /**
     * Builds a structured JSON error object so the JavaScript error callback
     * receives a machine-readable code in addition to a human-readable message.
     */
    private JSONObject buildError(String code, String message) {
        JSONObject error = new JSONObject();
        try {
            error.put("code", code);
            error.put("message", message);
        } catch (JSONException ignored) {
            // JSONObject.put(String, Object) cannot fail for String values
        }
        return error;
    }
}